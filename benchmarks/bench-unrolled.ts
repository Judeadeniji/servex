...
function compileUnrolled(handlers: Handler<Context>[]): (ctx: Context) => Promise<Response | undefined> {
...
async function testCorrectness() {
  ...
  console.log("Normal chain (3):", r1 instanceof Response ? "✓ Response" : "✗ " + r1);

  // Test 2: short-circuit at position 0 (auth-style)
  let afterAuth = false;
  const chainAuth = [
    async (ctx: any, next: any) => new Response("Unauthorized", { status: 401 }),
    async (ctx: any) => { afterAuth = true; return new Response("OK"); }
  ];
  const fnAuth = compileUnrolled(chainAuth as any);
  const rAuth = await fnAuth(ctx);
  console.log("Short-circuit at 0:", rAuth instanceof Response && rAuth.status === 401 ? "✓" : "✗", "afterAuth ran:", afterAuth);

  // Test 3: short-circuit at position 1
  const chainMid = [
    async (ctx: any, next: any) => { await next(); },
    async (ctx: any, next: any) => new Response("Stopped"),
    async (ctx: any) => new Response("Never reached"),
  ];
  const fnMid = compileUnrolled(chainMid as any);
  const rMid = await fnMid(ctx);
  console.log("Short-circuit at 1:", rMid instanceof Response && (await rMid.text()) === "Stopped" ? "✓" : "✗");
...
async function runBench() {
  ...
  for (const len of [1, 3, 8, 20]) {
    ...
    console.log(`Speedup:               ${(nonJitTime / unrolledTime).toFixed(2)}x`);
    console.log("");
  }
}
