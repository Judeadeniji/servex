...
const standardApp = createServer().post("/rpc/test", (c) => c.json({ ok: true }));

// Warmup
for (let i = 0; i < 5000; i++) {
    await client.test({});
    await standardApp.fetch(new Request("http://localhost/rpc/test", { method: "POST", body: JSON.stringify({}), headers: { "content-type": "application/json"} }));
}
