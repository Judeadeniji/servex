...
describe("queries()", () => {
	it("should return array of query parameters", async () => {
		...
		const res = await app.fetch(
			new Request("http://localhost/search?tags=action&tags=comedy"),
		);
	...
	it("should return record of query parameters", async () => {
		...
		const res = await app.fetch(
			new Request("http://localhost/search?tags=action&tags=comedy&sort=desc"),
		);
	...
	it("should return null for missing parameter", async () => {
		...
		const res = await app.fetch(
			new Request("http://localhost/search?tags=action"),
		);
