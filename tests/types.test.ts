...
import type { NormalisePath, AbsolutePath } from "../src/router/types";
...
type InferBase<T> = T extends ServeXRouter<any, any, infer B> ? B : never;
...
type _TestBaseLiteralCanonical = Expect<Equal<typeof apiApp.basePath, "/api/v1">>;
type _TestBaseGenericCanonical = Expect<Equal<InferBase<typeof apiApp>, "/api/v1">>;

// Trailing-slash input: normalised to "/api"
const trailingApp = createServer({ basePath: "/api/" });
type _TestBaseLiteralTrailing = Expect<Equal<typeof trailingApp.basePath, "/api">>;

// No-leading-slash input: normalised to "/api"
const noLeadingApp = createServer({ basePath: "api" });
type _TestBaseLiteralNoLeading = Expect<Equal<typeof noLeadingApp.basePath, "/api">>;
...
type ExpectedRoutedSchema =
  { "/api/users": { GET: Response & TypedResponse<{ users: never[] }, 200, "json"> } } &
...
const statusApp = createServer({ basePath: "/api" }).delete("/users/:id", (c) => c.text("Deleted", 204));
