...
describe("SonicRouter - Precedence & Specificity", () => {
  function getWinner(routes: string[], url: string): string | undefined {
    ...
    for (const route of routes) {
      router.addRoute({ method: "GET", path: route, data: { name: route } as any });
    }
    const matched = router.match("GET", url);
    return (matched?.data as any)?.name;
  }

  it("Static vs Param (Param registered first)", () => {
    expect(getWinner(["/users/:id", "/users/profile"], "/users/profile")).toBe("/users/profile");
  });

  it("Static vs Param (Static registered first)", () => {
    expect(getWinner(["/users/profile", "/users/:id"], "/users/profile")).toBe("/users/profile");
  });

  it("Param vs Wildcard", () => {
    expect(getWinner(["/files/*", "/files/:name"], "/files/config.json")).toBe("/files/:name");
  });
  ...
  it("Deeper Static Tie-breaker", () => {
    ...
    expect(getWinner(["/:category/items", "/books/:id"], "/books/items")).toBe("/books/:id");
  });

  it("Length Tie-breaker (Wildcards)", () => {
    expect(getWinner(["/api/*", "/api/users/*"], "/api/users/123")).toBe("/api/users/*");
  });

  it("Registration Order (Identical Shape)", () => {
    expect(getWinner(["/post/:id", "/post/:slug"], "/post/hello")).toBe("/post/:id");
  });
  ...
  it("Multiple Params vs Static Segment", () => {
    expect(getWinner(["/api/:version/:resource", "/api/v1/users"], "/api/v1/users")).toBe("/api/v1/users");
  });
});
