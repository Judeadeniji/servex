...
describe("Router", () => {
  ...
  it("should handle PUT, PATCH, and DELETE methods", async () => {
    ...
    let res = await app.fetch(new Request("http://localhost/items/1", { method: "PUT" }));
    expect(await res.text()).toBe("PUT 1");

    res = await app.fetch(new Request("http://localhost/items/2", { method: "PATCH" }));
    expect(await res.text()).toBe("PATCH 2");

    res = await app.fetch(new Request("http://localhost/items/3", { method: "DELETE" }));
