...
### Recent Experiment: Single/Localized Closures (Phase 1.5)
An attempt was made to optimize memory overhead by avoiding the `N` closures generated by the recursive `dispatch` function. A "single shared closure" and later a "localized closure inside switch cases" architecture was implemented.
* The single-closure approach relied on an integer bitmask (`called |= (1 << i)`) to track duplicate `next()` calls without allocating arrays.
* While it boosted a bugged version to 555k ops/sec, correctly scoping the closure to prevent cross-handler `next()` contamination required reverting to localized closures.
* **Result:** 342k ops/sec (short chain) and 100k ops/sec (long chain). The long chain performed ~19% *slower* than the native uncompiled loop, confirming that closure allocation inside `switch` cases defeats the JIT benefits.

### Why it can't be fixed with more codegen (The Microtask Barrier)
The true bottleneck is the number of `await` boundaries crossed per request. An N-handler chain requires N microtask ticks regardless of how the call sites are compiled, because each `await` suspends execution and re-enters the microtask queue. This is a runtime invariant that no JS-level `new Function` trick can escape. 

### What remains from Phase 1
`compileHandlerChain` is kept because:
- It has correct short-circuit behavior (documented and tested in `tests/compiler.test.ts`).
- It successfully separated string generation (`buildCompilerSource`) from evaluation, enabling AST-free unit testing.
- It provides the foundation for the **sync-handler fast path** (Phase 1.5).

---

## Phase 1.5 — Sync-Handler Fast Path (Synchronous Dispatch)
...
|-------------------------------------|------------------|------------|
| Native Short Chain (3 handlers)     |       345,378 ops/sec |  2895.38ms |
| Compiled Short Chain (3 handlers)   |       615,958 ops/sec |  1623.49ms |
