...
- Preserve monomorphic inline caches (ICs). Ensure objects passed to the same call site always have the same shape.
- Use the spread operator (`...`) or explicit object literals for object cloning and merging instead of `Object.assign()`. `Object.assign()` introduces measurable function call overhead.
...
- Do not unroll property assignments into flat, massive ASTs in generated code. This causes TurboFan bailouts.
  ...
  ```ts
  // Wrong — megamorphic, causes IC pollution
  const p: any = {};
  p.id = segments[1];
  p.slug = segments[2];
  return p;
...
- Do not use `instanceof` checks in any per-request code path.
