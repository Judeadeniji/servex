...
describe("Middleware: CORS", () => {
	...
	it("should dynamically allow multiple origins from array", async () => {
		...
		expect(resC.headers.get("Access-Control-Allow-Origin")).toBe(
			"https://a.com",
		);
	});
	...
	it("should dynamically allow origin using a function", async () => {
		...
		const resMatch = await app.fetch(
			new Request("http://localhost/", {
				headers: { Origin: "https://sub.example.com" },
			}),
		);
		expect(resMatch.headers.get("Access-Control-Allow-Origin")).toBe(
			"https://sub.example.com",
		);

		const resNoMatch = await app.fetch(
			new Request("http://localhost/", {
				headers: { Origin: "https://attacker.com" },
			}),
		);
		expect(resNoMatch.headers.get("Access-Control-Allow-Origin")).toBe(
			"https://example.com",
		);
	});
});
