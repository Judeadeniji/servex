...
import { rateLimiter, MemoryRateLimitStore } from "./index";
...
describe("Middleware: Rate Limiter", () => {
  ...
  it("should use a custom key generator to separate limits", async () => {
    ...
    const reqUser1 = new Request("http://localhost/", { headers: { "x-user-id": "user1" } });
    const reqUser2 = new Request("http://localhost/", { headers: { "x-user-id": "user2" } });
