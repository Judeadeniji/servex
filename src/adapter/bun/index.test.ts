...
describe("Bun adapter integration (adapted from Elysia)", () => {
	it("handle query guard", async () => {
		...
		app.get("/works-with", queryValidator, (c) => c.text("Works" + c.valid("query").a));
	...
	it("handle static response with onRequest and onError", async () => {
		...
		const app = createServer()
			.onError((error, c) => {
				...
				return c.text("handled", error instanceof HttpException ? error.statusCode : 500);
			})
			.onRequest((c) => {
	...
	it("handle non-ASCII path", async () => {
		...
		const response = await fetch(
			`http://localhost:${server.port}/สวัสดี`
		);
		const text = await response.text();
