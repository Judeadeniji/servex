...
const getCryptoKey = async (secret: string, alg: Algorithm, usages: KeyUsage[]): Promise<CryptoKey> => {
...
export const sign = async (payload: any, secret: string, alg: Algorithm = "HS256"): Promise<string> => {
  const header = { alg, typ: "JWT" };
  const encodedHeader = bufToBase64Url(encodeText(JSON.stringify(header)).buffer);
  const encodedPayload = bufToBase64Url(encodeText(JSON.stringify(payload)).buffer);

  const dataToSign = `${encodedHeader}.${encodedPayload}`;
  const key = await getCryptoKey(secret, alg, ["sign"]);
  
  const signature = await crypto.subtle.sign("HMAC", key, encodeText(dataToSign));
  return `${dataToSign}.${bufToBase64Url(signature)}`;
};

export const verify = async (token: string, secret: string, alg: Algorithm = "HS256"): Promise<any> => {
  ...
  const isValid = await crypto.subtle.verify("HMAC", key, signatureBuf, encodeText(dataToSign));
