...
describe("Helpers: JWT", () => {
	...
	it("should decode a token without verifying", async () => {
		...
		expect((decoded.header as Record<string, unknown>).alg).toBe("HS256");
		expect((decoded.payload as Record<string, unknown>).userId).toBe(123);
	});
	...
	it("should fail to verify with wrong secret", async () => {
		...
		expect(verify(token, "wrong-secret")).rejects.toThrow(
